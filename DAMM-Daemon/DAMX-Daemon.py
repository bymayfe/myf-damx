...
class DAMXManager:
    ...
    def get_thermal_profile(self) -> str:
        ...
        if hasattr(self, 'keyboard_monitor') and self.keyboard_monitor and getattr(self.keyboard_monitor, 'smart_fan_enabled', False):
            return "smart"
    ...
    def set_thermal_profile(self, profile: str) -> bool:
        ...
        if profile == "smart":
            if hasattr(self, 'keyboard_monitor') and self.keyboard_monitor:
                self.keyboard_monitor.smart_fan_enabled = True
                self.keyboard_monitor.last_smart_fan_level = -1
            curr = self._read_file("/sys/firmware/acpi/platform_profile").strip()
            if curr != "balanced":
                return self._write_file("/sys/firmware/acpi/platform_profile", "balanced")
            return True

        if hasattr(self, 'keyboard_monitor') and self.keyboard_monitor:
            self.keyboard_monitor.smart_fan_enabled = False
    ...
    def get_thermal_profile_choices(self) -> List[str]:
        ...
        res = choices.split() if choices else []
        if "balanced" in res and "smart" not in res:
            res.append("smart")
        return res
...
class DAMXDaemon:
    ...
    def setup(self):
        ...
        try:
            ...
            self.manager.keyboard_monitor = self.keyboard_monitor
